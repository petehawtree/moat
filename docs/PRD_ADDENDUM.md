...
- each returned text block carries its own citation list, which gives
  **claim-level** granularity for free. The `stub`'s original design —
  one prose blob plus a bag of citations — cannot enforce "every claim is
  cited", because nothing says which claim a citation belongs to.
...
### A15.4 Decision: citations are a table, not a JSON column

`ai_analysis.citations` (JSON) is kept as the **raw record of what the API
returned** — an audit trail, never queried for logic. The durable index is a
first-class `citations` table with a foreign key to `filings`.

**Why:** JSON in a column cannot answer the three questions that "future
recall" actually means:
...
3. **"What evidence did the brief I read in March actually stand on?"** —
   which requires citation rows to be **append-only**, marked `superseded_at`
   rather than overwritten. An analysis that is silently rewritten under a
   stable `run_id` is not a record of anything.
...
**Rules that do not bend:** an `unresolved` citation is *recorded as
unresolved and marks its analysis stale* — it is never deleted, and it is
never re-pointed at a different span that happens to score well. A citation
that quietly changes what it points at is worse than one that admits it is
broken.
...
**Decision:** `cache_key = sha256(analysis_type ‖ prompt_version ‖ model ‖
norm_version ‖ sorted(doc_sha256...))`, with every component also stored in
its own column. Storing only the digest makes "why did this regenerate?"
