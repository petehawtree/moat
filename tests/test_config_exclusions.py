...
def test_ai_analysis_exclusion_is_20_plus_googl():
    ...
    assert len(config.AI_ANALYSIS_KNOWN_EXCLUDED_TICKERS) == 21
...
def test_valuation_exclusion_is_20_plus_bkng():
    ...
    assert len(config.VALUATION_KNOWN_EXCLUDED_TICKERS) == 21
...
def test_committee_exclusion_is_the_union_of_both_stages():
    ...
    assert len(config.COMMITTEE_KNOWN_EXCLUDED_TICKERS) == 22  # 18 + AMT + SBAC + GOOGL + BKNG
    assert {"GOOGL", "BKNG", "AMT", "SBAC"} <= config.COMMITTEE_KNOWN_EXCLUDED_TICKERS
